...
function loadTaskActivities(formData) {
    $("#attach-card")
        .empty().html('<div class="col text-center"><div class="spinner-border" role="status" aria-hidden="true"></div> Loading...</div>'
        );
    $.ajax({
        type: "GET",
            ...
            for (x in data) {
                for (var y = 0; y < data[x].length; y++) {
                    ...
                    if (data[x][y].running_task == 0) {
                        /*check for, whether task is ended or not*/
                    ...
                    var cardInner = $(
                        "<div class='card card-style-1'  /><div class=''></div>"
                    );
                    cardInner.append(cardHeader);

                    var cardBody = $("<div class='card-body module line-clamp' />");
                    ...
                    var actionPlay = $(     // play action for not running tasks
                        '<a href="' +
                        timeTrackerBaseURL +
                        "user/start_timer?id=" +
                        data[x][y].id +
                    ...
                    var actionStop = $(     // stop button for the tasks that are running..
                        '<a href="' +
                        timeTrackerBaseURL +
                        "user/stop_timer?id=" +
                        data[x][y].id +
                    ...
                    if (data[x][y].running_task == 1 && data[x][y].start_time != null) {
                        ...
                        document.getElementsByClassName("title").innerText = data[x][y].task_name;
                    		...
                    		/*cardInner.css("background-image", "linear-gradient(to right, #ffffff, #f7f8fa, #f0f1f6");
                            cardHeader.css("background-image", "linear-gradient(to right, #ffffff, #f7f8fa, #f0f1f6");
                            cardBody.css("background-image", "linear-gradient(to right, #ffffff, #f7f8fa, #f0f1f6");
                            cardFooter.css("background-image", "linear-gradient(to right, #ffffff, #f7f8fa, #f0f1f6");*/
