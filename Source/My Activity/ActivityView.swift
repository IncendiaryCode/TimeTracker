...
class ActivityView: UIView, UITableViewDelegate, UITableViewDataSource, CalendarViewDataSource,
	...
	override func awakeFromNib() {
        ...
        tblActivities.register(UINib(nibName: "TaskInfoCell", bundle: nil),
							   forCellReuseIdentifier: "taskInfoCell")
	...
	func calendar(_ calendar: CalendarView, didScrollToMonth date: Date) {
		...
		if date.month != dateCurrentMonth.month {
			...
			let strScrolledDate = date.addingTimeInterval(TimeInterval(-TimeZone.current
				.secondsFromGMT())).getStrDate()
	...
	@objc func btnDayChartPressed(sender: UIGestureRecognizer) {
		...
		if let indexPath = tblActivities.indexPathForSelectedRow {
			...
			if let cell = tblActivities.cellForRow(at: indexPath) {
				(cell as! TaskDetailsCell).gradientLayer.colors =
					[]
			}
		...
		if let button = sender.view as? UIButton {
			...
			UIView.animate(withDuration: 0.3, animations: {
				...
				_ in
				let cell = self.tblActivities.cellForRow(at: indexPath) as! TaskDetailsCell
...
//			}
	...
	func tableView(_ tableView: UITableView, didHighlightRowAt indexPath: IndexPath) {
		if let cell = tableView.cellForRow(at: indexPath) as? TaskDetailsCell {
			// Highlight touch.
...
//		}
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		let cell = tableView.dequeueReusableCell(withIdentifier: "taskInfoCell",
												 for: indexPath) as! TaskDetailsCell
